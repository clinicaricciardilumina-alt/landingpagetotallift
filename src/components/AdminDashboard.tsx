                ...
                <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 h-fit">
                  ...
                  <div className="space-y-6 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                    {availableImages.map(img => {
                      ...
                      return (
                        <div key={img} className="group border-2 border-gray-50 rounded-2xl p-5 hover:border-[#0066A1]/30 transition-all bg-gray-50/50">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            {isHero && (
                              <span className="bg-[#0066A1] text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">Immagine Hero</span>
                            )}
                          </div>
                          
                          <div className="aspect-video rounded-xl overflow-hidden bg-gray-200 relative">
                            <img src={`/images/${img}`} className="w-full h-full object-cover" alt="preview" />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4">
                               <button 
                                onClick={() => setSettings({...settings, hero_image: `/images/${img}`})}
                                className="bg-white text-[#0066A1] px-4 py-2 rounded-lg font-bold text-xs"
                               >
                                Imposta come Hero
                               </button>
                            </div>
