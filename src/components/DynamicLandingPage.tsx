...
import LandingReviewsBlock from "./reviews/LandingReviewsBlock";
...
export default function DynamicLandingPage() {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          // Init tracking pixel/GA
          try {
            const tracking = await import("../lib/trackingService");
            tracking.initTrackingForLanding(l, settings);
          } catch (e) {
            console.warn("Errore init tracking:", e);
          }
        } catch (e) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Tracking evento Lead (Facebook Pixel + Google)
      if (landing.trackFormSubmission !== false) {
        try {
          const tracking = await import("../lib/trackingService");
          tracking.trackEvent("Lead", {
            content_name: landing.internalName,
            content_category: landing.category,
            value: 1,
            currency: "EUR",
            lead_id: created.id,
          });
        } catch (trackErr) {
          console.warn("Errore tracking lead:", trackErr);
        }
      }
  ...
  const handleBookSlot = async (slot: BookingSlot) => {
    ...
    try {
      ...
      // Tracking evento Schedule
      if (landing?.trackBookingMade !== false) {
        try {
          const tracking = await import("../lib/trackingService");
          tracking.trackEvent("Schedule", {
            content_name: landing?.internalName || "",
            slot_id: slot.id,
            slot_date: slot.date,
            slot_time: slot.time,
          ...
          console.warn("Errore tracking booking:", e);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* RECENSIONI */}
      {landing.reviewsEnabled && (
        <LandingReviewsBlock landing={landing} primaryColor={primary} />
      )}
