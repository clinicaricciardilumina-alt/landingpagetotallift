...
export interface AppSettings {
  ...
  // ----- Cloudinary (archivio immagini) -----
  cloudinaryCloudName?: string;   // Es. "drnrhvjd1"
  cloudinaryUploadPreset?: string; // Es. "landing_uploads"
  cloudinaryFolder?: string;       // Cartella destinazione (default "landing_pages")
...
// =====================================================
// IMAGE LIBRARY (archivio immagini Cloudinary)
// =====================================================
...
export interface LibraryImage {
  id: string;
  name: string;                       // Nome leggibile (es. "Sala operatoria 1")
  url: string;                        // URL Cloudinary
  publicId: string;                   // ID Cloudinary per delete/transform
  category: ImageCategory;
  tags: string[];                     // Tag liberi (es. ["sbiancamento", "casi", "donna"])
  width?: number;
  height?: number;
  format?: string;                    // "jpg", "png", ecc.
  bytes?: number;                     // Dimensione file
  description?: string;
  altText?: string;                   // Per SEO/accessibilità
  uploadedAt: string;
}
